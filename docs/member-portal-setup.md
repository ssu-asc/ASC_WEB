...
각 일정에는 **하루 종일** 여부가 있다. 하루 종일을 켜면 시간 입력 대신 날짜만 선택한다.

- 일반 일정 — 선택한 시작일 `00:00`부터 종료일 `23:59:59.999`까지 저장하고 UI에서는 시간을 숨긴다.
- 프로젝트 제출 — 제출 시작일 `00:00`부터 마감일 `23:59:59.999`까지로 처리해 기존 제출 가능/지각 판정 로직을 그대로 사용한다.
- 시작일과 종료일이 같으면 그 날짜 한 날 전체 일정이다.
- 반복 일정의 각 발생 회차에도 `all_day`가 그대로 materialize된다.
- 기존 일정은 migration 013에서 `all_day=false`가 기본이므로 기존 시각 표시를 그대로 유지한다.
- `전체 일정 → 일괄 수정`에서도 각 행의 `하루 종일`을 개별 전환할 수 있다.
...
운영진은 `전체 일정 → 일괄 수정`에서 여러 일정/프로젝트 회차를 체크해 한 화면에서 제목·시작·종료/마감을 수정하고 한 번에 저장할 수 있다. 선택 항목 전체를 `+N일/-N일` 이동하는 보조 기능도 제공한다. `선택 삭제`와 이중 확인이 필요한 `전체 삭제`도 제공한다. 일반 일정은 삭제하고, 프로젝트 회차는 기존 삭제 API로 비활성화한다. **이미 제출 기록이 있는 프로젝트 회차는 삭제가 차단**되어 기록이 보존된다. 반복 생성 회차가 삭제 대상에 포함되면 해당 반복 규칙도 함께 종료해 다음 materialize에서 회차가 다시 생기지 않게 한다. 기존 row의 optimistic `version`을 그대로 사용하므로 다른 운영진이 먼저 수정한 항목은 stale 저장으로 덮어쓰지 않는다. 반복 규칙으로 생성된 특정 회차를 일괄 수정하면 **그 발생 회차만** 바뀌며 반복 규칙 자체는 그대로 남는다. 반복 전체를 바꾸려면 별도의 `반복 규칙 수정`을 사용한다.
