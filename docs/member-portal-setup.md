...
- `/member/operations/submissions` — 회차별 프로젝트 현황/검토
- `/member/operations/teams` — 학기 고정 팀 편성
- `/member/operations/members` — 스프레드시트형 회원관리 + 회원별 임시 비밀번호 초기화
...
프로젝트 회차는 내부적으로 `assignments`에 저장된다.
...
`개인 ↔ 팀 반복`은 영구 반복 규칙을 저장하지 않는다. 지정한 횟수만큼 독립된 assignment를 한 트랜잭션에서 생성한다. 이후 시험주 등 특정 회차만 별도로 수정/비활성화할 수 있다.
...
운영진 현황은 `회원 × 모든 과제` 장표가 아니라 **회차 우선**으로 본다.
...
각 회차에서 제출 수/대상 수, 지각, 수정요청, 승인 수를 확인하고 상태/검색 필터를 사용할 수 있다.
...
- 여러 행을 선택해 권한/학기 활동/계정 상태를 로컬에서 일괄 변경할 수 있다.
- 입력 중에는 DB에 쓰지 않는다.
...
```text
로그인 아이디 | 이름 | 권한 | 이번 학기 활동 | GitHub | 계정 상태
```

헤더를 포함해 붙여넣는 경우 아래 영문 alias도 인식한다.

```text
member_id, name, role, semester_active, github_username, account_active, temporary_password
```
...
공개 `/apply`는 계속 `public/data/apply.json`의 모집 링크를 사용한다. 내부 resource link를 수정해도 공개 모집 페이지는 자동 변경되지 않는다.
